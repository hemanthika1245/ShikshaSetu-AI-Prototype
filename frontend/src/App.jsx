  ...
  return (
    <div className="login-shell">
      ...
      <div className="login-showcase">
        ...
        <div className="showcase-stats"><div><b>3</b><span>Languages</span></div><div><b>JWT</b><span>Auth</span></div><div><b>∞</b><span>Possibilities</span></div></div>
      </div>
    </div>
  );
}
  ...
  return (
    <main>
      ...
      <section className="impact">
        <div><div className="impact-number">3</div><div>Target tribal languages</div></div>
...
function StudentMode() {
  ...
  async function check() {
    try {
      const r = await fetch(`${API}/quiz`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({answer,expected:"Sunlight"})});
      const d=await r.json(); setFeedback(`${d.correct?"⭐ ":""}${d.feedback}`);
    } catch {
      setFeedback(answer.trim().toLowerCase()==="sunlight" ? "⭐ Great job! You understood the concept." : "Good try. The answer is sunlight.");
    }
  }

  function playQuestion() {
    if ("speechSynthesis" in window) speechSynthesis.speak(new SpeechSynthesisUtterance("Why do plants need sunlight?"));
  }

  return <main className="student page">
    <div className="student-top"><div className="eyebrow">STUDENT MODE • CLASS 3</div><div className="streak"><Star size={16} fill="currentColor"/> 4 day learning streak</div></div>
    <div className="student-card">
      <div className="plant-illustration">🌱</div>
      <span className="lesson-tag">SCIENCE • PLANTS</span>
      <h1>Why do plants need sunlight?</h1>
      ...
      <div className="answer-box"><div className="answer-head"><span>Your answer</span><button className={recording?"recording":"record"} onClick={listen}><Mic size={18}/>{recording?"Listening…":"Speak"}</button></div><input value={answer} onChange={e=>setAnswer(e.target.value)} placeholder="Type or speak your answer…"/><button className="primary full" onClick={check}>Check my answer <ArrowRight size={17}/></button></div>
